...
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>GNSS Multi-Node Dashboard</title>
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<style>
body{margin:0;font-family:sans-serif;display:flex;height:100vh}
#map{flex:2}#log{flex:1;overflow-y:auto;padding:10px;background:#111;color:#0f0;font-family:monospace;font-size:12px}
table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #333;padding:4px;text-align:left}
</style></head><body>
<div id="map"></div>
<div id="log"><h3 style="color:#fff;">Live Nodes</h3>
<table><thead><tr><th>Node</th><th>Time</th><th>Lat</th><th>Lon</th><th>Alt</th></tr></thead>
...
async function poll() {
  try {
    ...
    for (const [id, data] of Object.entries(nodes)) {
      ...
      if (!markers[id]) {
        markers[id] = L.circleMarker([lat, lon], {radius:8,color:colors[id]||'green'}).addTo(map).bindTooltip(id,{permanent:true});
      } else { markers[id].setLatLng([lat, lon]); }
